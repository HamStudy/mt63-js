...
export function dspWindowBlackman3(dspPhase: number): number {
  return (
    0.35875 +
    0.48829 * Math.cos(dspPhase) +
    0.14128 * Math.cos(2 * dspPhase) +
    0.01168 * Math.cos(3 * dspPhase)
  );
}
...
export function dspWinFirI(
  ...
  for (i = 0; i < len; i++) {
    ...
    if (time === 0) {
      ...
      shapeValue =
        (Math.sin(uppOmega * time) - Math.sin(lowOmega * time)) / time;
    }
...
export function winFirQ(
  ...
  for (i = 0; i < len; i++) {
    ...
    if (time === 0) {
      ...
      shapeValue =
        (-Math.cos(uppOmega * time) + Math.cos(lowOmega * time)) / time;
    }
...
export function dspLowPass2Coeff(integLen: number): {
  w1: number;
  w2: number;
  w5: number;
} {
...
export function dspLowPass2(
  ...
  w2: number,
  w5: number
): { mid: dspCmpx; out: dspCmpx };
export function dspLowPass2(
  input: number | dspCmpx,
  mid: number | dspCmpx,
  out: number | dspCmpx,
  w1: number,
  w2: number,
  w5: number
): { mid: number | dspCmpx; out: number | dspCmpx } {
  if (typeof input === 'number') {
    ...
    return {
      ...
      out: new dspCmpx(outC.re + w5 * diffRe, outC.im + w5 * diffIm),
    };
  }
}

export function dspFindMax(
...
export function dspPower(
  X: Float32Array | Float64Array | number[] | dspCmpx[],
  len: number
): number;
export function dspPower(
  I: Float32Array | Float64Array | number[],
  Q: Float32Array | Float64Array | number[],
  len: number
): number;
export function dspPower(
  x: number | dspCmpx | Float32Array | Float64Array | number[] | dspCmpx[],
  y?: number | Float32Array | Float64Array | number[],
  z?: number
): number {
  if (typeof x === 'number') {
    let resp = x * x;
...
export function dspScalProd(
  x1OrRe: dspCmpx | number,
  x2OrIm: dspCmpx | number,
  x?: dspCmpx
): number {
...
export function dspFindMaxPower(
  data: dspCmpx[],
  len: number
...
export function dspSelFitAver(
  ...
  selThres: number,
  loops: number
): { aver: number; rms: number; sel: number };
export function dspSelFitAver(
  data: dspCmpx[],
  len: number,
  selThres: number,
  loops: number
): { aver: dspCmpx; rms: number; sel: number };
export function dspSelFitAver(
...
export function dspCmpxMult(a: dspCmpx, b: dspCmpx): dspCmpx {
  return new dspCmpx(a.re * b.re - a.im * b.im, a.re * b.im + a.im * b.re);
}

export function dspCmpxMultConj(a: dspCmpx, b: dspCmpx): dspCmpx {
  // Multiply a by conjugate of b
  return new dspCmpx(a.re * b.re + a.im * b.im, a.im * b.re - a.re * b.im);
}
